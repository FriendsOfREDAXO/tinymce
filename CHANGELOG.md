...
* **MediaPlace-Erkennung nutzte noch das alte globale JS-Objekt `MP3`:** `window.rex5MediaplaceBridge` in `assets/scripts/base.js` prüfte auf `MP3.open()`/`MP3.openFile()`, MediaPlace hat sein globales Overlay-Objekt seit Version 2.0.0 aber in `MP` umbenannt. Dadurch öffnete sich bei aktuellem MediaPlace wieder der klassische Medienpool-Popup statt des Overlays. Auf `MP` aktualisiert.
...
* **`link_assume_external_targets: 'https'` als Folgefehler zu #175:** Das Link-Plugin von TinyMCE 8 präfixt URLs ohne erkanntes Schema stillschweigend mit `https://`. Für Mediapool-Pfade wie `/media/datei.pdf` entstand dadurch nach „Speichern" im Link-Dialog die kaputte URL `https:///media/datei.pdf` (leerer Host). Fix: Default-Guard in `base.js` (`link_assume_external_targets: false`, sofern nicht explizit gesetzt), Seed-Profile umgestellt, Profil-Assistent erzeugt die Option nicht mehr (das harmlose `link_default_protocol: 'https'` bleibt erhalten).
