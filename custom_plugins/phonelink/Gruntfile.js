...
module.exports = (grunt) => {
  ...
  grunt.initConfig({
    ...
    concat: {
      license: {
        ...
        files: {
          'dist/phonelink/plugin.js': [
            'src/text/license-header.js',
            scratchPluginMinPath
          ],
          'dist/phonelink/plugin.min.js': [
            'src/text/license-header.js',
            scratchPluginMinPath
          ]
        }
    ...
    copy: {
      css: {
        files: [
          { src: [ 'CHANGELOG.txt', 'LICENSE.txt' ], dest: 'dist/phonelink', expand: true }
        ]
      }
    },
  ...
  grunt.registerTask('version', 'Creates a version file', () => {
    grunt.file.write('dist/phonelink/version.txt', BUILD_VERSION);
  });
